...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "查询某个农产品的历史价格，用于回答最近价格怎么样、近期走势、X天前价格、上个月价格、某段时间价格趋势等问题。不指定日期时默认返回最近30天数据",
            "parameters": {
                "type": "object",
                "properties": {
                    "product_name": {
...
- 问"最近/近期/这段时间/走势/怎么样"→ 用 get_price_history（不传日期，自动返回近30天）
- 问"X天前/上周/上个月/某段时间"→ 用 get_price_history 并传入对应日期
